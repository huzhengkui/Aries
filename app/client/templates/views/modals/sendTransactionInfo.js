...
Template['views_modals_sendTransactionInfo'].helpers({
    /**
    Calculates the fee used for this transaction in.okc.r

    @.okc.d (estimatedFee)
    */
    'estimatedFee': function() {
        if(this.estimatedGas && this.gasPrice)
            return EthTools.formatBalance(new BigNumber(this.estimatedGas, 10).times(new BigNumber(this.gasPrice, 10)), '0,0.0[0000000] unit', .okc.r');
    }
});