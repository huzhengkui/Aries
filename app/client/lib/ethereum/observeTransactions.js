...
addTransaction = function(log, from, to, value){
    ...
    var block = web3.okc.getBlock(log.blockNumber, false, function(err, block){
        if(!err) {

            web3.okc.getTransaction(log.transactionHash, function(err, transaction) {
                if(!err && transaction) {
                    web3.okc.getTransactionReceipt(log.transactionHash, function(err, receipt){
...
var updateTransaction = function(newDocument, transaction, receipt){
    ...
    if(receipt && transaction) {
        ...
        if(!newDocument.contractAddress && receipt.contractAddress) {
            web3.okc.getCode(receipt.contractAddress, function(e, code) {
                if(!e && code.length > 2) {
    ...
    if(newDocument.outOfGas) {
        ...
        if(EthAccounts.findOne({address: newDocument.from})) {
            web3.okc.getBalance(newDocument.from, newDocument.blockNumber, function(e, now){
                if(!e) {
                    web3.okc.getBalance(newDocument.from, newDocument.blockNumber-1, function(e, then){
    ...
    */
    var checkTransactionConfirmations = function(tx){
        ...
        if(!tx.confirmed && tx.transactionHash) {
                ...
                if(!e) {
                    ...
                    if(confirmations < ethereumConfig.requiredConfirmations && confirmations >= 0) {
                        ...
                        web3.okc.getTransaction(tx.transactionHash, function(e, transaction){
                            web3.okc.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                            ...
                            web3.okc.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                                if(!e) {
                                    ...
                                    if(!transaction || !transaction.blockNumber) {
                                        ...
                                        web3.okc.getBlock(transaction.blockNumber, function(e, block) {
                                            if(!e) {
            ...
            var filter = web3.okc.filter('latest').watch(function(e, blockHash) {
